...
import { IconTienda, IconFormacion, IconWhatsapp } from "@/components/icons";
...
export default async function TiendaPage() {
  ...
  const [{ data: categorias }, { data: packs }] = await Promise.all([
    supabase.from("tienda_categorias").select("*").eq("is_active", true).order("sort_order", { ascending: true }),
    supabase.from("packs").select("*").eq("is_active", true).order("sort_order", { ascending: true }),
  ]);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 text-center">
      ...
      {packs && packs.length > 0 && (
        <div className="mt-16">
          <h2 className="mb-2 flex items-center justify-center gap-2 font-serif text-2xl">
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3">
            {packs.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="flex flex-col rounded-sm border border-black/10 bg-white p-5 text-center shadow-sm"
                >
