...
import { createColorimetriaPost, updateColorimetriaPost } from "@/app/admin/actions";
import { BUTTON_PRIMARY, INPUT } from "@/lib/ui";
import RichTextEditor from "@/components/admin/RichTextEditor";
import type { ColorimetriaPost } from "@/types/database";

export default function ColorimetriaPostForm(props: { post?: ColorimetriaPost }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editing && props.post) {
        await updateColorimetriaPost(props.post.id, formData);
        router.push("/admin/colorimetria");
        return;
      }
      await createColorimetriaPost(formData);
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="grid grid-cols-1 gap-3">
      ...
      <button type="submit" disabled={uploading || saving} className={BUTTON_PRIMARY}>
        {saving ? "Guardando..." : editing ? "Actualizar artículo" : "Publicar artículo"}
      </button>
    </form>
  );
}
