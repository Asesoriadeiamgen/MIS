...
import { createAgenda, updateAgenda } from "@/app/admin/actions";
...
export default function AgendaForm(props: { agenda?: Agenda }) {
  const router = useRouter();
  const editing = !!props.agenda;
  const formRef = useRef<HTMLFormElement>(null);
  const [imageUrls, setImageUrls] = useState<string[]>(props.agenda?.image_urls || []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      if (editing && props.agenda) {
        await updateAgenda(props.agenda.id, formData);
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      <input
        ...
        defaultValue={props.agenda?.name}
        required
        className={INPUT}
      />
      <PriceField
        name="base_price"
        defaultValue={props.agenda?.base_price}
        placeholder="Precio base"
      />
      <textarea
        name="description"
        placeholder="Descripción"
        defaultValue={props.agenda?.description ?? ""}
        className={`${INPUT} sm:col-span-2`}
      />
      <div className="sm:col-span-2">
        <label className="block text-xs text-gray-500 mb-1">
          Fotos o videos del diseño (podés subir varios; videos hasta 20MB)
        </label>
        {imageUrls.length > 0 && (
      ...
      >
        {saving ? "Guardando..." : editing ? "Actualizar agenda" : "Crear agenda"}
      </button>
    </form>
  );
}
